...
namespace LibraryManagementSystem.Controllers
{
    public class BookIssuesTablesController : Controller
    {
        ...
        public ActionResult Create()
        {
            ...
            List<DatabaseLayer.BooksTable> books = db.BooksTables.ToList();
            IEnumerable<SelectListItem> bookItems = books.Select(book =>
                new SelectListItem
                {
                    Value = book.BookID.ToString(),
                    Text = book.BookTitle // Adjust according to the properties in DatabaseLayer.BookTable
        });

            ViewBag.BookID = bookItems;
            List<DatabaseLayer.EmployeeTable> employees = db.EmployeeTables.ToList();
            IEnumerable<SelectListItem> employeeItems = employees.Select(emp =>
                new SelectListItem
                {
                    Value = emp.EmployeeID.ToString(),
                    Text = emp.FullName // Adjust according to the properties in DatabaseLayer.EmployeeTable
                });

            ViewBag.EmployeeID = employeeItems;
...
#pragma warning restore CS0472 // The result of the expression is always the same since a value of this type is never equal to 'null'
            ...
            SelectList bookSelectList = new SelectList(bookItems, "Value", "Text", bookIssuesTable.BookID);
            ViewBag.BookID = bookSelectList;
            List<DatabaseLayer.EmployeeTable> employees = db.EmployeeTables.ToList();
            IEnumerable<SelectListItem> employeeItems = employees.Select(emp =>
                new SelectListItem
                {
            ...
            SelectList employeeSelectList = new SelectList(employeeItems, "Value", "Text", bookIssuesTable.EmployeeID);
